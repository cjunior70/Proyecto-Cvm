    ...
    <div style={{ position: 'absolute', left: '-9999px', top: '0' }}>
      <div ref={downloadRef} style={{ width: '1150px', padding: '60px', background: '#f4f7f9', fontFamily: 'sans-serif' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          {/* ENCABEZADOS DE JORNADA */}
          <div style={{ display: 'flex', marginBottom: '10px' }}>
            <div style={{ width: '260px' }}></div> {/* Espacio vacío sobre los roles */}
            <div style={{ display: 'flex', gap: '25px', flex: 1 }}>
              {servicios.map((s) => (
                <div key={s.Id} style={{ 
                  flex: 1, background: colors.sky, padding: '15px', 
                  borderRadius: '15px 15px 0 0', color: 'white', textAlign: 'center' 
                }}>
                  <div style={{ fontSize: '11px', fontWeight: 'bold', opacity: 0.9 }}>{s.Jornada}</div>
                  <div style={{ fontSize: '18px', fontWeight: 'bold' }}>{s.Tipo}</div>
                </div>
              ))}
            </div>
          </div>

          {/* FILAS DE ROLES Y SERVIDORES */}
          {datosFlyer.areas.map((area, index) => (
            <div key={area.Id} style={{ 
              display: 'flex', 
              alignItems: 'center', 
              marginBottom: '10px',
              // Color de fondo alternado para las filas para guiar la vista
              background: index % 2 === 0 ? 'rgba(0,0,0,0.02)' : 'transparent',
              borderRadius: '10px'
            }}>
              
              {/* Etiqueta del Rol (Izquierda) */}
              <div style={{ 
                width: '260px', height: '65px', display: 'flex', alignItems: 'center',
              ...
              {/* Servidores Asignados (Derecha) */}
              <div style={{ display: 'flex', gap: '25px', flex: 1 }}>
                {servicios.map((s) => {
                  const nombre = datosFlyer.asignaciones[area.Id]?.[s.Id] || '';
                  return (
                    <div key={s.Id} style={{ 
                      flex: 1, height: '65px', background: colors.glass, 
        ...
        <div style={{ marginTop: '30px', textAlign: 'center', color: '#a0aec0', fontSize: '12px' }}>
          GENERADO AUTOMÁTICAMENTE • {new Date().getFullYear()}
        </div>
      </div>
    </div>
  </>
